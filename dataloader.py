...
class SingletonDataset(Singleton):
    ...
    def __init__(self,
        ...
        if pretext_target_col is not None:
            ...
            assert (not pretext_with_label) or self.dataset.columns[-1] != pretext_target_col

            if pretext_with_label:
                self.dataset = self.dataset.iloc[:, :-1]
        
            assert f"pretext_target_{pretext_target_col.strip()}" not in self.dataset.columns
            self.dataset[f"pretext_target_{pretext_target_col.strip()}"] = self.dataset[pretext_target_col].copy()
        
        self.dataset_x, self.dataset_y = self.dataset.iloc[:, :-1], self.dataset.iloc[:, [-1]]
        self.data_size = len(self.dataset_x)

        assert min_cat_count > 0
        self.min_cat_count = min_cat_count if isinstance(min_cat_count, int) else  int(self.data_size * min_cat_count)# recommend 2%-5% of data points

        self.stats_x = data_stats(self.dataset_x, min_cat_count=self.min_cat_count)
        ...
        self.train_dataset, self.validate_dataset = self._split_dataset(validate_split=validate_split)

        self.train_dataset_size, self.validate_dataset_size = len(self.train_dataset), len(self.validate_dataset)

        self.train_dataset_x, self.train_dataset_y = self.train_dataset.iloc[:, :-1], self.train_dataset.iloc[:, [-1]]
        self.validate_dataset_x, self.validate_dataset_y = self.validate_dataset.iloc[:, :-1], self.validate_dataset.iloc[:, [-1]]

        self._get_tasktype()

    def _split_dataset(self, validate_split: float = 0.2) -> Tuple[pd.DataFrame, pd.DataFrame]:
        # Shuffle the DataFrame
        dataset_shuffled = self.dataset.sample(frac=1.0, replace=False, random_state=42)
    ...
    def _get_tasktype(self):
        ...
        if isinstance(target_stats, CategoricalStats):
            ...
            self.target_map = {key: index for index, key in enumerate(target_stats.valid_cats.keys())}
        else:
    ...
    def __iter__(self):
        ...
        while True:
            ...
            for n in range(num_batches):
                ...
                y_tok = y if self.sdata.target_map is None else y.map(lambda x: self.sdata.target_map[x])
